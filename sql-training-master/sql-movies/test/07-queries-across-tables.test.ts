...
describe("Queries Across Tables", () => {
  ...
  it(
    ...
    async done => {
      ...
      count(case WHEN m.original_title='Life' THEN 1 ELSE null END) as count
      FROM ${MOVIE_ACTORS} ma 
      INNER JOIN ${ACTORS} a
      ON ma.actor_id = a.id
      INNER JOIN ${MOVIES} m
      ON ma.movie_id = m.id and m.original_title = 'Life'`;
  ...
  it(
    ...
    async done => {
      ...
      SELECT 
      g.genre as genre,
      count(case WHEN mr.rating = 5 THEN 1 ELSE null END) as five_stars_count
