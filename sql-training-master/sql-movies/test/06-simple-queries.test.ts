...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      ...
      SELECT 
      count(
        case WHEN budget > 100000000 THEN 1 ELSE null END 
        AND 
        case WHEN release_date > '2009' THEN 1 ELSE null END
        ) as count 
      FROM ${MOVIES}`;
      const result = await db.selectSingleRow(query);
  ...
  it(
    ...
    async done => {
      const query = `
      SELECT 
      count(case WHEN homepage LIKE 'https%' THEN 1 ELSE null END) as count 
      FROM ${MOVIES}`;
      const result = await db.selectSingleRow(query);
