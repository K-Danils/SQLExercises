...
const insertMovieGenres = (
  ...
  let filtredGenres = genreRows.filter(genreRow => genres.includes(genreRow.genre));
  return `INSERT INTO ${MOVIE_GENRES} (movie_id, genre_id) values ` + 
  filtredGenres.map(genre => `(${movieId}, ${genre.id})`).join(",");
};
...
const insertMovieActors = (
  ...
  let fileteredActors = actorRows.filter(actorRow => actors.includes(actorRow.full_name));
  return `INSERT INTO ${MOVIE_ACTORS} (movie_id, actor_id) values ` + 
  fileteredActors.map(actor => `(${movieId}, ${actor.id})`).join(",");
};
...
const insertMovieDirectors = (
  ...
  let filteredDirectors = directorRows.filter(directorRow => directors.includes(directorRow.full_name));
  return `INSERT INTO ${MOVIE_DIRECTORS} (movie_id, director_id) values ` + 
  filteredDirectors.map(director => `(${movieId}, ${director.id})`).join(",");
};
...
const insertMovieKeywords = (
  ...
  let filteredKeywords = keywordRows.filter(keywordRow => keywords.includes(keywordRow.keyword));
  return `INSERT INTO ${MOVIE_KEYWORDS} (movie_id, keyword_id) values ` + 
  filteredKeywords.map(keyword => `(${movieId}, ${keyword.id})`).join(",");
};
...
const insertMovieProductionCompanies = (
  ...
  let filteredCompanies = productionCompanyRows.filter(companyRow => productionCompanies.includes(companyRow.company_name));
  return `INSERT INTO ${MOVIE_PRODUCTION_COMPANIES} (movie_id, company_id) values ` + 
  filteredCompanies.map(company => `(${movieId}, ${company.id})`).join(",");
};
